...
// Qt Quick Controls style selection (must be set before creating the application).
// NOTE: We intentionally *avoid* including QQuickStyle here because some CI Qt
// installations ship the QML plugin but not the C++ header/framework.
// Using the environment variable keeps us compatible and still lets us force
// a non-native style when we customize control internals.
#include <QByteArray>
...
{
    // Qt Quick Controls on macOS uses a native style by default. That style does
    // not support customization of some controls (e.g. overriding Button::contentItem).
    // Force a non-native style so our custom QML controls work consistently.
    // NOTE: this must be set *before* creating Q(Gui)Application.
    if (qEnvironmentVariableIsEmpty("QT_QUICK_CONTROLS_STYLE"))
        qputenv("QT_QUICK_CONTROLS_STYLE", QByteArrayLiteral("Basic"));

    // Qt 6 always has High-DPI scaling and high-DPI pixmaps enabled.
    // Keep these attributes only for Qt 5.
#if QT_VERSION < QT_VERSION_CHECK(6, 0, 0)
#  ifdef Q_OS_ANDROID
...
#  endif
    ...
    QMLBridge qmlBridge;
    // Expose the singleton used for Keypad <-> Emu communication.
    the_qml_bridge = &qmlBridge;
    QQmlEngine::setObjectOwnership(the_qml_bridge, QQmlEngine::CppOwnership);

    // Register QMLBridge for Keypad<->Emu communication
    qmlRegisterSingletonType<QMLBridge>(
        "Firebird.Emu", 1, 0, "Emu",
